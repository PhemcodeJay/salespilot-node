...
const { User, Product, Category, Sale, SalesAnalytics, Inventory, Report, sequelize } = require('../models'); // Assuming Sequelize models
...
async function calculateProductCategoryMetrics() {
    const salesCategoryData = await sequelize.query(`
        SELECT
...
async function getPreviousYearRevenue(date) {
    ...
    const salesData = await SalesAnalytics.findOne({ where: { date: { [Op.like]: `${year}%` } } });
    return salesData ? salesData.revenue : 0;
}
...
async function checkIfReportExists(date) {
    const existingReport = await SalesAnalytics.findOne({ where: { date } });
...
async function saveReport(date, reportData, existingReport = null) {
    if (existingReport) {
        await SalesAnalytics.update(reportData, { where: { id: existingReport.id } });
    } else {
        await SalesAnalytics.create({ ...reportData, date });
    }
}
...
async function getNotifications() {
    const inventoryNotifications = await Inventory.findAll({
        where: {
            [Op.or]: [
                { available_stock: { [Op.lt]: 10 } },
                { available_stock: { [Op.gt]: 1000 } }
            ]
        },
        include: [{ model: Product, attributes: ['image_path'] }],
        order: [['last_updated', 'DESC']]
    });

    const reportNotifications = await Report.findAll({
        where: {
            [Op.or]: [
                { revenue: { [Op.gt]: 10000 } },
                { revenue: { [Op.lt]: 1000 } }
